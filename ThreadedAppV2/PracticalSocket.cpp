...
#ifdef WIN32
int TCPServerSocket::getInterfaces(std::vector<std::string> &addrs) {
    ...
    for (int i = 0; i < nNumInterfaces; ++i) {
        ...
        //iaddr = pAddress->sin_addr.S_un.S_addr;

        //pAddress = (sockaddr_in *) & (InterfaceList[i].iiNetmask);
        //netmask = pAddress->sin_addr.S_un.S_addr;

        //unsigned long netaddr = iaddr & netmask;
        //unsigned long netbcast = netaddr | (~netmask);

        //pAddress->sin_addr.S_un.S_addr = netbcast;

        u_long nFlags = InterfaceList[i].iiFlags;
        if (nFlags & IFF_UP) {
            addrs.push_back(inet_ntoa(pAddress->sin_addr));
        }
    }
